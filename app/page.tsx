...
export default function Home() {
  const { iterations, startNew } = useAnimate(solver, nlp, {});

  const handleNewInitialGuess = (x: Matrix) => {
    startNew(x);
  };
  return (
    <main className="flex h-dvh w-dvw bg-gray-900 text-gray-100">
      ...
      <section className="flex min-h-0 min-w-0 flex-1 flex-col gap-2 p-2">
        <OptimizationProblem iterations={iterations} handleNewInitialGuess={handleNewInitialGuess}/>
